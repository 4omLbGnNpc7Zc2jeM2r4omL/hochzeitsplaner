...
function createGuestsChart(data) {
    ...
    // Äußerer Ring: Gesamtanzahl Gäste
    const outerData = [gesamt];
    const outerLabels = [`Gäste gesamt (${gesamt})`];
    const outerColors = ['#007bff']; // Gleiche Farbe wie Budget-Chart
...
function createBudgetChart(data) {
    ...
    // Äußerer Ring: Gesamtbudget (100%)
    const outerData = [planned];
    const outerLabels = [`Gesamtbudget (${formatCurrency(planned)})`];
    const outerColors = ['#007bff'];
    
    // Innerer Ring: Verteilung Ausgegeben/Verbleibt
    const innerData = [];
    const innerLabels = [];
    const innerColors = [];
    ...
    if (spent > 0) {
        ...
        innerColors.push(overspent > 0 ? '#dc3545' : '#28a745');
    }
...
// Event Listeners
document.addEventListener('DOMContentLoaded', function() {
    console.log('Hochzeitsplaner Web Application geladen');
