...
function updateFixkostenDisplay() {
    ...
    Object.keys(currentFixkosten).forEach(name => {
        ...
        col.innerHTML = `
            <div class="d-flex justify-content-between align-items-center p-3 border rounded" style="background: rgba(255, 255, 255, 0.7); border-color: var(--wedding-gold) !important; border-radius: 12px !important;">
                <div>
                    <strong style="color: var(--wedding-text-dark);">${name}</strong>
                    <div class="small" style="color: var(--wedding-text);">Fixkosten</div>
                </div>
                <div class="d-flex align-items-center">
                    <span class="me-3 fw-bold" style="color: var(--wedding-gold);">${formatEuro(betrag)}</span>
                    <div class="btn-group">
                        <button class="btn btn-sm btn-wedding-secondary" onclick="editFixkosten('${name}', ${betrag})" title="Bearbeiten">
                            <i class="fas fa-edit"></i>
                        </button>
                        <button class="btn btn-sm btn-outline-danger" onclick="removeFixkosten('${name}')" title="Löschen" style="border-color: #dc3545; color: #dc3545;">
